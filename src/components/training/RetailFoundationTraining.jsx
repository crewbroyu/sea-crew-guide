...
import { ArrowRight, CheckCircle2, ChevronDown, ExternalLink, MapPin, ShoppingBag, Volume2 } from 'lucide-react'
...
export default function RetailFoundationTraining({ onStartSimulation, onStartQuestions, initialProgress, onProgressChange }) {
  ...
  const updateDay = (day, patch) => {
    setProgress((current) => {
      ...
      nextDay.completedAt = shadowingDone && challengeDone ? nextDay.completedAt || new Date().toISOString() : null
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="divide-y divide-slate-100 rounded-lg border border-slate-200 bg-white px-4 shadow-sm sm:px-5">
        {retailFoundationDays.map((day, dayIndex) => {
          const dayProgress = progress.days?.[day.id] || {}
          const isActive = activeDayId === day.id
          const isCompleted = Boolean(dayProgress.completedAt)
          return (
            <article key={day.id}>
              <button type="button" onClick={() => setActiveDayId(isActive ? '' : day.id)} className="flex min-h-16 w-full items-center gap-3 py-3 text-left">
              ...
              {isActive && (
                <div className="pb-6 sm:pl-12">
                  <div className="rounded-lg bg-blue-50 p-4"><p className="text-xs font-semibold text-blue-700">TODAY&apos;S MISSION</p><p className="mt-1 text-sm font-medium leading-6 text-blue-950">{day.mission}</p></div>
                  ...
                  <section className="mt-5">
                    ...
                    <div className="mt-3 divide-y divide-slate-100 border-y border-slate-100">
                      {day.vocabulary.map((item) => <div key={item.term} className="grid gap-2 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div><div className="flex flex-wrap items-baseline gap-2"><p className="font-semibold text-slate-950">{item.term}</p><p className="font-mono text-xs text-blue-700">{item.ipa}</p><p className="text-xs text-slate-500">{item.meaning}</p></div><p className="mt-1 text-sm leading-6 text-slate-700">{item.example}</p></div><button type="button" onClick={() => speakEnglish(`${item.term}. ${item.example}`)} title={`Listen to ${item.term}`} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-blue-700 hover:bg-blue-50"><Volume2 size={17} /></button></div>)}
                    </div>
                  </section>

                  <section className="mt-5 rounded-lg border border-slate-200 p-4"><div className="flex items-center gap-2"><ShoppingBag size={17} className="text-blue-700" /><h3 className="text-sm font-bold text-slate-950">SHIFT KNOWLEDGE</h3></div><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-700">{day.knowledge.map((item) => <li key={item}>• {item}</li>)}</ul></section>

                  <div className="mt-5"><PhraseShadowingPractice phrases={day.serviceLines.map((item) => item.line)} phraseCues={day.serviceLines.map((item) => item.cue)} practice={dayProgress.shadowing || {}} onPracticeChange={(shadowing) => updateDay(day, { shadowing })} requiredPhraseRepetitions={3} requireListenBeforeRecord title="LISTEN AND SHADOW" description="Listen to each line and record it three complete times." /></div>
                  <GuestChallengePractice role={day.challenge.role} prompt={day.challenge.prompt} challenge={dayProgress.guestChallenge || {}} locked={!dayProgress.shadowing?.completedAt} onChallengeChange={(guestChallenge) => updateDay(day, { guestChallenge })} />
                  <EdgeReadAloudHint />

                  {isCompleted && dayIndex < retailFoundationDays.length - 1 && <button type="button" onClick={() => setActiveDayId(retailFoundationDays[dayIndex + 1].id)} className="mt-4 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700">Continue to Day {day.day + 1}<ArrowRight size={16} /></button>}
                </div>
              )}
            </article>
          )
