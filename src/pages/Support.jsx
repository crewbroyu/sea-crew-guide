...
export default function Support() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
      <header className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-2xl px-5 pb-7 pt-12"><button type="button" onClick={() => navigate('/')} className="mb-5 inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"><ArrowLeft size={16} />返回首页</button><p className="text-sm font-medium text-blue-700">CrewPathGuide 支持中心</p><h1 className="mt-2 text-2xl font-semibold text-slate-950">遇到问题，直接告诉我们</h1><p className="mt-2 text-sm leading-6 text-slate-600">权益、AI 训练、功能问题和产品建议都会进入同一处理队列。</p></div></header>
      <main className="mx-auto max-w-2xl space-y-5 px-5 py-6">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-700"><p>想先确认产品边界、数据保存和人工开通规则？<button type="button" onClick={() => navigate('/service-info')} className="ml-1 font-semibold text-blue-700 underline underline-offset-2">查看服务与数据说明</button></p></section>
