                ...
                <div className="pb-6 sm:pl-12">
                  ...
                  {showLessonStep(1) && <section className="mt-5">
                    ...
                    <div className="mt-3 divide-y divide-slate-100 border-y border-slate-100">
                      {day.vocabulary.map((item) => <div key={item.term} className="grid gap-2 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div><div className="flex flex-wrap items-baseline gap-2"><p className="font-semibold text-slate-950">{item.term}</p><p className="font-mono text-xs text-blue-700">{item.ipa}</p><p className="text-xs text-slate-500">{item.meaning}</p></div><p className="mt-1 text-sm leading-6 text-slate-700">{item.example}</p></div><div className="flex gap-2"><button type="button" onClick={() => speakEnglish(`${item.term}. ${item.example}`, { position: 'retail' })} title={`Listen to ${item.term}`} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-blue-700 hover:bg-blue-50"><Volume2 size={17} /></button><button type="button" onClick={() => onToggleSavedLine?.({ text: item.example, cue: `Vocabulary · ${item.term}`, day: day.day, dayId: day.id })} title="Save this example" aria-label={`Save example for ${item.term}`} className={`inline-flex h-9 w-9 items-center justify-center rounded-lg border ${savedLines.some((line) => line.text === item.example) ? 'border-blue-300 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-500'}`}><Bookmark size={17} fill={savedLines.some((line) => line.text === item.example) ? 'currentColor' : 'none'} /></button></div></div>)}
                    </div>
                  </section>}
                  ...
                  {showLessonStep(3) && <div className="mt-5"><PhraseShadowingPractice position="retail" phrases={day.serviceLines.map((item) => item.line)} phraseCues={day.serviceLines.map((item) => item.cue)} practice={dayProgress.shadowing || {}} onPracticeChange={(shadowing) => updateDay(day, { shadowing })} requiredPhraseRepetitions={3} requireListenBeforeRecord title="LISTEN AND SHADOW" description="Listen to each line and record it three complete times." savedLines={savedLines} onToggleSavedLine={(text, cue) => onToggleSavedLine?.({ text, cue, day: day.day, dayId: day.id })} /></div>}
                  {showLessonStep(4) && <GuestChallengePractice position="retail" role={day.challenge.role} prompt={day.challenge.prompt} challenge={dayProgress.guestChallenge || {}} locked={!dayProgress.shadowing?.completedAt} onChallengeChange={(guestChallenge) => updateDay(day, { guestChallenge })} />}
