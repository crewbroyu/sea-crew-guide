...
export default function BarServerScenarioTraining({ jobKey = 'bar_server' }) {
  const navigate = useNavigate()
  const simulator = useMemo(() => getJobSimulator(jobKey), [jobKey])
  const scenarios = useMemo(() => getJobScenarios(jobKey), [jobKey])
  const skills = useMemo(() => getJobSkills(jobKey), [jobKey])
  ...
  useEffect(() => {
    ...
    const loadTrainingData = async () => {
      try {
        const [nextProfile, nextHistory, nextDraft] = await Promise.all([getMyScenarioProfile(jobKey), getMyScenarioHistory(jobKey), getMyInProgressScenarioSession(jobKey)])
        if (!active) return
        setProfile(nextProfile)
        setHistory(nextHistory)
        setDraft(nextDraft)
        if (nextProfile?.recommended_scenario_id) setScenarioId(nextProfile.recommended_scenario_id)
      } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <main className="mx-auto max-w-3xl space-y-5 px-5 py-6">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-medium text-blue-700">{scenario.episode}</p><h2 className="mt-1 text-xl font-semibold text-slate-950">{scenario.title}</h2></div><select value={scenario.id} onChange={(event) => resetScenario(event.target.value)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800"><option value={scenario.id}>Change simulation</option>{scenarios.filter((item) => item.id !== scenario.id).map((item) => <option key={item.id} value={item.id}>{item.episode} · {item.title}</option>)}</select></div>
